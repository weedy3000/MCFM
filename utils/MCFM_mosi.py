...
class CompetitiveSelectionMechanism(nn.Module):
    ...
    def forward(self, text_cls, audio_cls, text_feature, audio_feature, labels):
        ...
        loss1 = nn.MSELoss(reduction='none')(logits1.squeeze(), labels)  # 设定reduction参数为none，损失不加和，直接返回各样本损失
        loss2 = nn.MSELoss(reduction='none')(logits2.squeeze(), labels)

        cls_logits1 = self.class_sys1(o_last_hidden_tensor1)
        cls_logits2 = self.class_sys2(o_last_hidden_tensor2)
        cls_loss1 = nn.CrossEntropyLoss()(cls_logits1, la)
        cls_loss2 = nn.CrossEntropyLoss()(cls_logits2, la)
        ...
        if self.args.histo_type != "none":
            ...
            self.histo_cache1.append((cls_logits1.argmax(dim=-1).eq(la),
                                      loss1.detach().clone().to('cpu'),
                                      o_last_hidden_tensor1.detach().clone().to("cpu")))
            self.histo_cache2.append((cls_logits2.argmax(dim=-1).eq(la),
        ...
        final_loss = nn.MSELoss()(final_logits.squeeze(), labels)
        cls_final_loss = nn.CrossEntropyLoss()(cls_final_logits, la)
        ...
        return [total_loss, cls_final_loss, regression_loss, final_loss, adv_loss * 0.2, diff_loss], final_logits
...
class MCFM(nn.Module):
    ...
    def forward(self, text_ids, text_mask, audio_ids, audio_mask, labels):
        ...
        loss, select_feature = self.competitiver(text_cls, audio_cls, text_feature, audio_feature, labels)

        return loss, select_feature
