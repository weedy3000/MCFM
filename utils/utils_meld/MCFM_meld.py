...
from transformers import BertModel, BertTokenizer, Data2VecAudioModel,RobertaModel
...
class Orthogonal_attention(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, channels, height):
        ...
        self.height = height
        # self.conv = nn.Conv1d(in_channels=768,out_channels=self.channels,kernel_size=3)
        # Gram-Schmidt 变换初始化
    ...
    def forward(self, share,private):
        ...
        # if  W != self.height:
        #     conv_out1 = nn.functional.adaptive_avg_pool1d(query,self.height) #(bs,out_channel,height)
        #     conv_out2 = nn.functional.adaptive_avg_pool1d(key,self.height)
        # conv_out1 = conv_out1.unsqueeze(2) #(bs,out_channel,1,height)
        # conv_out2 = conv_out2.unsqueeze(2)
...
class Multimodal_Feature_Extracter(nn.Module):
    def __init__(self, text_model, audio_model, hidden_dim=768):
        ...
        self.audio_model = Data2VecAudioModel.from_pretrained(audio_model)
        # 文本处理器
    ...
    def forward(self, text_ids, text_mask, audio_ids, audio_mask):
        ...
        audio_out = self.audio_model(audio_ids, audio_mask)
        audio_last_hidden_state = audio_out.last_hidden_state #(bs,len,dim)
        # print(audio_last_hidden_state.shape)
...
class CompetitiveSelectionMechanism(nn.Module):
    def __init__(self, args,critic_head_num=1, input_dim=768, hidden_dim=768):
        ...
        if self.args.histo_type == "cat":
            self.referee = nn.Linear(hidden_dim * 6, 3)
            self.class_fc = nn.Linear(hidden_dim*6,7)
        else:
            self.referee = nn.Linear(hidden_dim * 4, 3)
            # self.referee = MLP(
            #     input_size=hidden_dim * 4,
            #     hidden_size=hidden_dim,
            #     output_size=1, num_layers=2, dropout=args.hidden_dropout_prob,
            #     layer_norm=False)
    ...
    def forward(self, text_cls,audio_cls,text_feature, audio_feature,labels_s,labels_e):
        ...
        loss1 = nn.CrossEntropyLoss(reduction='none')(logits1.squeeze(), labels_s) # 设定reduction参数为none，损失不加和，直接返回各样本损失
        loss2 = nn.CrossEntropyLoss(reduction='none')(logits2.squeeze(), labels_s)
        
        cls_logits1 = self.class_sys1(o_last_hidden_tensor1)
        cls_logits2 = self.class_sys2(o_last_hidden_tensor2)
        cls_loss1 = nn.CrossEntropyLoss()(cls_logits1, labels_e)
        cls_loss2 = nn.CrossEntropyLoss()(cls_logits2, labels_e)
        ...
        if self.args.histo_type != "none":
            ...
            self.histo_cache1.append((cls_logits1.argmax(dim=-1).eq(labels_e),
                                      loss1.detach().clone().to('cpu'),
                                      o_last_hidden_tensor1.detach().clone().to("cpu")))
            self.histo_cache2.append((cls_logits2.argmax(dim=-1).eq(labels_e),
        ...
        final_loss = nn.CrossEntropyLoss()(select_logits.squeeze(), labels_s)
        emotion_loss = nn.CrossEntropyLoss()(cls_final_logits, labels_e)
        
        # total_cls_loss = (cls_loss1 + cls_loss2)*0.5 + 0.5 * cls_final_loss
        sentiment_loss = (loss1.mean() + loss2.mean()) * 0.5
        total_loss = (loss1.mean() + loss2.mean()) * 0.5 + final_loss + 0.2 * adv_loss + emotion_loss + diff_loss

        return [total_loss,final_loss,sentiment_loss,emotion_loss,adv_loss*0.2,diff_loss],select_logits,cls_final_logits
    ...
    def get_historical_h(self, syshidden, histo):
        ...
        # 采用transform进行特征那个加权融合
#         combined_input = torch.cat((syshidden.unsqueeze(1), histo.unsqueeze(0).repeat(syshidden.size(0), 1, 1)), dim=1) #(bs,3,dim)
#         # 使用 Transformer 编码器处理当前输入和历史信息
#         # Transformer 编码器的输入需要是 (seq_length, batch_size, dim) 的形状
#         combined_input = combined_input.transpose(0, 1)  # (seq_length, batch_size, dim)
#         attention_mask = torch.ones(combined_input.size(1), combined_input.size(0)).to(self.args.device)
#         # print(attention_mask.shape)
#         transformer_output = self.transformer(combined_input, src_key_padding_mask=(attention_mask == 0))
        
#         # 取 Transformer 输出的第一个 token 作为历史信息的表示
#         historical_h = transformer_output[0,:,:]
        # print(historical_h.shape)
        return historical_h
...
class MultimodalEmotionAnalyzer_ortho(nn.Module):
    def __init__(self,args):
        super(MultimodalEmotionAnalyzer_ortho, self).__init__()
        self.args = args
    ...
    def forward(self,text_ids,text_mask,audio_ids,audio_mask,labels_s,labels_e):
        text_cls,audio_cls,text_feature, audio_feature = self.feature_extractor(text_ids, text_mask, audio_ids, audio_mask)

        loss,select_logits,cls_final_logits = self.competitiver(text_cls,audio_cls,text_feature, audio_feature,labels_s,labels_e)

        return loss,select_logits,cls_final_logits